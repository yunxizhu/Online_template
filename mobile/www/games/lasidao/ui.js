...
window.LasidaoUi = (function () {
  ...
  function hideCardTip() {
    ...
    tip.hidden = true;
    tip.innerHTML = '';
    tip.classList.remove('has-preview', 'is-pinned', 'is-perm-tip');
    tip._lasTipText = '';
    tip._lasTipImg = '';
  ...
  function maybeShowMercenaryToast(game, meId) {
    ...
    } else if (currentMercenaryDieIndex(merc) >= 0) {
      const idx = currentMercenaryDieIndex(merc);
      showTurnToast(
        t('lasidao.eventMercenaryPlaceDieToast', {
          face: merc.roll[idx],
          cur: idx + 1,
          total: merc.roll.length,
        }),
        1800
      );
    }
  ...
  function updateDiceHint() {
    ...
    if (isMercenaryRollMode(lastGame, lastMeId)) {
      hint.textContent = t('lasidao.eventMercenaryRollHint');
      return;
    }
    if (isMercenaryPlaceMode(lastGame, lastMeId)) {
      if (diceAnim.stage === 'rolling') {
        ...
        const face = mercenaryCurrentFace(lastGame);
        if (!selectedTarget) {
          hint.textContent = t('lasidao.eventMercenaryPickTarget', {
            face: face || '?',
          });
        } else {
  ...
  function renderGameLog(game) {
    ...
    if (len === lastLogRenderLen && log.children.length > 0) return;
    const newest = len > 0 ? rows[len - 1] : null;
    if (
      len === lastLogRenderLen + 1 &&
      newest &&
      log.children.length > 0 &&
      log.children.length <= 60
    ) {
      const li = document.createElement('li');
      li.textContent = newest.text;
      log.insertBefore(li, log.firstChild);
      while (log.children.length > 60) {
        log.removeChild(log.lastChild);
      }
      lastLogRenderLen = len;
      return;
    }
    lastLogRenderLen = len;
    log.innerHTML = '';
    for (const row of rows.slice().reverse().slice(0, 60)) {
      const li = document.createElement('li');
  ...
  function pickAreaTarget(areaKey, num) {
    if (isMercenaryPlaceMode(lastGame, lastMeId)) {
      const face = mercenaryCurrentFace(lastGame);
      if (face == null || Number(num) !== face) return;
      selectedFace = face;
  ...
  function getBoardSlotInteraction(game, meId, areaKey, num, slotInfo, tiles, workers) {
    ...
    const mercFace = mercPlace ? mercenaryCurrentFace(game) : null;
  ...
  function renderRollWrap(game, meId) {
    ...
    if (remoteBtn) {
      ...
      remoteBtn.hidden = !(showProduce && hasCard);
    }
  }
  ...
  function renderMercenaryDice(game, meId) {
    ...
    if (diceAnim.stage === 'ready') {
      ...
      const face = roll[idx];
      if (
        diceAnim.finalDice.length !== 1 ||
        diceAnim.finalDice[0] !== face
      ) {
        diceAnim.finalDice = [face];
        diceAnim.finalBoosted = [false];
        selectedFace = face;
        selectedTarget = null;
        renderGroupedDice();
      }
      setDiceTitle(
        t('lasidao.eventMercenaryPlaceDie', {
          face,
          cur: idx + 1,
          total: roll.length,
        })
  ...
  function syncBuildConfirmBar(game, me) {
    const bar = $('las-act-confirm-bar');
    const playBtn = $('btn-las-build-play');
    const passBtn = $('btn-las-pass');
    if (!bar || !playBtn) return;

    const isBuild = game && game.phase === 'build';
    ...
    } else if (selectedPermanent === 'buyFunc') {
      playBtn.textContent = t('lasidao.confirmBuyFunc');
    } else {
      playBtn.textContent = t('lasidao.confirmUse');
    }

    playBtn.onclick = canPlay
      ? () =>
          confirmBuildHandSelection(
            lastGame || game,
            mePlayer(lastGame || game, me.id)
          )
      : null;
  }
  ...
  function renderFuncForm(game, me) {
    ...
    if (
      ...
      if (selectedPermanent === 'buildHouse') {
        ...
        tip.textContent = t('lasidao.breedFormHint', breedCostPayload(game, me));
      } else if (selectedPermanent === 'expand') {
        title.textContent = t('lasidao.permanentFormTitle', {
          label: t('lasidao.expandPermanent'),
        });
        tip.textContent = t('lasidao.expandPermanentTip', expandCostPayload(game));
      } else if (selectedPermanent === 'exchange') {
        title.textContent = t('lasidao.permanentFormTitle', {
  ...
  function render(game, net, opts) {
    ...
    if (game.phase === 'settle_act' && passBtn) {
      passBtn.hidden = true;
    ...
    const produceActions = $('las-produce-actions');
    if (produceActions && game.phase !== 'produce' && game.phase !== 'event_mercenary') {
  ...
  function maybePlaySettle(game) {
    ...
    if (produceFxBlocksSettle(game)) return;
    ...
    const finishUi = () => {
      settlePlaying = false;
      deferredHeavyRenderPending = false;
      // 保留 settleBoardFreeze / las-settling；Settlement 还未真正结束，
      // 提前 renderBoard 或清除 freeze 会导致 workers 在动画刚结束时就被抹掉。
      if (netRef) {
        netRef.sendAction('finishSettleAnim', {});
      }
      if (lastGame && lastMeId) {
        // 动画结束时不要主动重绘 board；让后续 server state 更新自然驱动渲染，
        // 避免用无 workers 的 settle state 把版面重建成空白。
        // renderBoard(lastGame, lastMeId);
    ...
    const run = async () => {
      ...
      // 收获演绎结束后再回收卡牌；骰子在 completeSettleAfterAnim 后自然消失
      await new Promise((r) => setTimeout(r, 400));
      const boardSnap = settleBoardFreeze || lastGame || game;
      if (LasFx && typeof LasFx.playRecycleBoard === 'function') {
        await LasFx.playRecycleBoard(boardSnap);
      }
    };
  ...
  async function confirmMercenaryPlace() {
    ...
    const idx = currentMercenaryDieIndex(merc);
    if (idx < 0 || !selectedTarget || selectedTarget.type !== 'area') return;
  ...
  function renderExchangeModal() {
    ...
    const hasMixer = (me.buildings || []).some(
    ...
    if (rateHint) {
      ...
      rateHint.textContent = caravan
        ...
        : hasMixer
          ? t('lasidao.exchangeHintMixer', { n: need })
          : exCount === 0
    ...
    function renderSide(listId, isFrom) {
      ...
      for (const r of RESOURCES) {
        ...
        if (isFrom) {
          ...
          const maxTo = hasMixer
            ? Math.floor(totalFrom / need)
            : RESOURCES.reduce((sum, res) => sum + Math.floor((exFromBatches[res] || 0) / need), 0);
    ...
    if (ready) {
      if (hasMixer) {
        if (totalFrom % need !== 0) {
          ...
          else if (need === 4 && RESOURCES.some(r => (exFromBatches[r] || 0) !== batchCount)) ready = false;
          else if (need >= 2 && RESOURCES.some(r => (exFromBatches[r] || 0) > batchCount)) ready = false;
        }
      } else {
        let totalBatch = 0;
        for (const r of RESOURCES) {
          const fc = exFromBatches[r] || 0;
          if (fc > 0 && fc % need !== 0) { ready = false; break; }
          totalBatch += fc / need;
        }
        if (ready) {
    ...
    if (confirmBtn) {
      ...
      if (ready) {
        ...
        confirmBtn.textContent = t('lasidao.exchangeNeedMatch', {
          left: hasMixer ? Math.floor(totalFrom / need) : totalFrom,
          right: totalTo,
        });
      } else {
    ...
    if (exConfirm) {
      exConfirm.onclick = () => {
            ...
            : exCount === 0
              ...
              : exCount === 1
                ...
                : exCount === 2
                  ? 2
                  : 1;
        const hasMixer = (m.buildings || []).some(
          (b) => b.built && b.buildType === 'mixer'
        );
        const totalFrom = RESOURCES.reduce((sum, r) => sum + (exFromBatches[r] || 0), 0);
        const totalTo = RESOURCES.reduce((sum, r) => sum + (exToBatches[r] || 0), 0);
        ...
        if (hasMixer) {
          ...
          if (need === 4 && RESOURCES.some(r => (exFromBatches[r] || 0) !== batch)) return;
          if (need >= 2 && RESOURCES.some(r => (exFromBatches[r] || 0) > batch)) return;
        } else {
          let totalBatch = 0;
          for (const r of RESOURCES) {
            const fc = exFromBatches[r] || 0;
            if (fc > 0 && fc % need !== 0) return;
            totalBatch += fc / need;
          }
          if (totalTo !== totalBatch) return;
        }
