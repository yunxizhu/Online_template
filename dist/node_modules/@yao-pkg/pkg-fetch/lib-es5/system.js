...
function detectAlpine() {
    ...
    const ldd = (0, child_process_1.spawnSync)('ldd').stderr?.toString();
    if (ldd == null) {
        return fs_1.default.readdirSync('/lib').some((file) => file.startsWith('libc.musl'));
    }
    if (/\bmusl\b/.test(ldd)) {
        return true;
    }
    const lddNode = (0, child_process_1.spawnSync)('ldd', [process.execPath]).stdout.toString();
...
function getArmUnameArch() {
    const uname = (0, child_process_1.spawnSync)('uname', ['-a']);
    if (uname.error) {
        return '';
    }
...
function getArmHostArch() {
    const cpu = fs_1.default.readFileSync('/proc/cpuinfo', 'utf8');
