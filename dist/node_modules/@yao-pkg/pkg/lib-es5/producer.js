...
function producer({ backpack, bakes, slash, target, symLinks, doCompress, nativeBuild, fallbackToSource, }) {
    return new Promise((resolve, reject) => {
        ...
        const makeCompressStream = doCompress === compress_type_1.CompressType.GZip
            ? zlib_1.createGzip
            : doCompress === compress_type_1.CompressType.Brotli
                ? zlib_1.createBrotliCompress
                : doCompress === compress_type_1.CompressType.Zstd
                    ? (0, compress_type_1.getZstdCompressStream)()
                    : null;
                ...
                if (stripes.length) {
                    ...
                    if (stripe.buffer) {
                        if (stripe.store === common_1.STORE_BLOB) {
                            ...
                            return (0, fabricator_1.fabricateTwice)(bakes, target.fabricator, snap, sourceBuffer, (error, buffer) => {
                                if (error) {
                                    const file = stripe.file ?? snap;
                                    if (fallbackToSource) {
                                        log_1.log.warn(`Failed to generate V8 bytecode for ${file}. Shipping source instead. Cause: ${error.message}`);
                                        stripe.store = common_1.STORE_CONTENT;
                                        stripe.buffer = sourceBuffer;
                                        return cb(null, pipeMayCompressToNewMeter((0, into_stream_1.default)(sourceBuffer)));
                                    }
                                    log_1.log.warn(`Failed to generate V8 bytecode for ${file}. Cause: ${error.message}. ` +
