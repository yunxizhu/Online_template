...
function stepDetect(record, marker, derivatives) {
    ...
    try {
        detector.detect(body, (node, trying, requireAliases) => {
            const { toplevel } = marker;
            let d = detector.visitorSuccessful(node, false, requireAliases);
            if (d) {
                if (d.mustExclude) {
                    return false;
                }
            ...
            d = detector.visitorNonLiteral(node, requireAliases);
            if (d) {
            ...
            d = detector.visitorMalformed(node, requireAliases);
            if (d) {
        ...
        }, record.file, (0, common_1.isESMFile)(record.file));
    }
    catch (error) {
...
class Walker {
    ...
    async appendBlobOrContent(task) {
        ...
        if (this.params.seaMode && task.store === common_1.STORE_BLOB) {
    ...
    needsSeaRead(record) {
        return (!!this.params.seaMode && ((0, common_1.isDotJS)(record.file) || (0, common_1.isESMFile)(record.file)));
    }
    stepPatch(record) {
        const patch = this.patches[record.file];
        if (!patch) {
    ...
    async step_STORE_ANY(record, marker, store) {
        ...
        if (store === common_1.STORE_BLOB ||
            ...
            if (!record.body) {
                ...
                if (store === common_1.STORE_BLOB || needsSeaRead) {
                    stepStrip(record);
                }
            }
