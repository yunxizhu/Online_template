...
function recycleBoard(game) {
  ...
  for (const num of ENVIRONMENT_SLOT_NUMBERS) {
    const env = envs[num];
    if (env && Array.isArray(env.stashCards)) {
      for (const c of env.stashCards) pushToDiscard(game, 'resource', c);
      env.stashCards = [];
    }
    if (env) pushToDiscard(game, 'environment', env);
  }
...
function takeBuildingCard(game, player, tile) {
  ...
  if (!assignBuildingSlot(player, neu)) {
    ...
    if (!hasUnbuilt) {
      ...
      pushLog(
        game,
        `${player.name} 建筑格已满且均已建造，弃置刚获得的建筑（入弃牌堆）`
      );
      return;
    }
...
function applyAction(game, playerId, action) {
  ...
  if (game.pendingRobberyPick) {
    if (type === 'robberyPick') {
      if (playerId !== game.pendingRobberyPick.targetId) {
        return { ok: false, error: '请由被抢劫玩家选择要交出的卡牌' };
      }
    ...
    return {
      ok: false,
      error: `等待 ${game.pendingRobberyPick.targetName} 选择要交出的卡牌`,
    };
  }
...
function actDiscardPendingBuild(game, player) {
  ...
  pushLog(
    ...
    pendingSource === 'demolition'
      ...
      : `${player.name} 弃置刚获得的建筑（入弃牌堆），保留现有建筑`
  );
  if (game.phase === 'settle_act') {
    maybeAdvanceAfterSettleActDiscard(game, player);
  }
...
function isStackAchievementKey(key) {
  ...
  if (STACK_ACHIEVEMENT_EXCLUDED_BUILD_TYPES.has(buildType)) return false;
  return true;
}
/** @deprecated 兼容旧导出/测试 */
...
function totalBuiltWorkshops(player) {
  let n = 0;
  for (const b of player.buildings || []) {
    if (b.built && b.buildType === 'produce') n += 1;
  }
  return n;
}
...
function workshopMasterScore(player, game) {
  return hasWorkshopMaster(player, game) ? WORKSHOP_MASTER_SCORE : 0;
}
...
/** @returns {boolean} 称号持有人是否变化 */
function resolveWorkshopMaster(game) {
  if (!game) return false;
  const prevId = game.workshopMasterPlayerId || null;
  const nextId = pickWorkshopMasterHolder(game);
  if (nextId === prevId) return false;
  game.workshopMasterPlayerId = nextId;
  const next = nextId ? playerById(game, nextId) : null;
  const prev = prevId ? playerById(game, prevId) : null;
  if (next && !prev) {
    pushLog(
...
function playerTitles(player, game) {
  ...
  if (hasWorkshopMaster(player, game)) {
    titles.push({
      id: WORKSHOP_MASTER_ID,
      stackKey: WORKSHOP_MASTER_STACK_KEY,
...
/** 抢劫：效果1 随机抢 2 资源；效果2 由对方选择 1 张未建建筑或功能卡交出 */
...
function useRobbery(game, player, payload) {
  ...
  if (!canRobberyStealCards(target)) {
    return {
      ok: false,
      error: `${target.name} 没有可交出的未建造建筑或功能卡`,
    };
  }
  if (game.pendingRobberyPick) {
    return { ok: false, error: '请先完成待处理的抢劫交牌' };
  }
  const pool = robberyCardLootPool(target);
  if (pool.length < 1) {
  ...
  pushLog(
    game,
    `${player.name} 抢劫：请 ${target.name} 选择一张未建造建筑或功能卡交出`
  );
...
function actCancelRobberyPick(game, player) {
  ...
  pushLog(game, `${player.name} 取消了「抢劫」`);
  return { ok: true };
}
...
function actRobberyPick(game, player, payload) {
  ...
  if (!option) return { ok: false, error: '请选择一张未建造建筑或功能卡' };
  ...
  const stepText =
    `${actor.name} 抢劫：${target.name} 交出一张${
...
/** 强化：将 1 枚未强化骰子强化（建造阶段）；已达上限 5 或全部已强化则无法发动 */
  ...
  if (deckKindOfTile(card) === 'building') {
    ...
    if (!assignBuildingSlot(player, neu)) {
      ...
      if (!hasUnbuilt) {
        ...
        pushLog(
          game,
          `${player.name} 建筑格已满，须弃置刚获得的建筑或先腾出空位`
        );
      } else {
        queuePendingBuildCard(player, neu);
        pushLog(
          game,
          `${player.name} 建筑格不足，请弃置一张未建造建筑以收下刚获得的建筑`
        );
      }
    } else {
      player.buildings.push(neu);
...
function actRedrawPick(game, player, payload) {
  ...
  if (pending.redrawCardId) {
    ...
    pushLog(
      game,
      `${player.name} 重抽：保留 1 张${keepKindText}，弃置其余 ${discardCount} 张`
    );
    ...
    pushLog(
      game,
      `${player.name} 购买功能卡：保留 1 张${keepKindText}，弃置其余 ${discardCount} 张`
    );
    ...
    pushLog(
      game,
      `${player.name} 抽牌：保留 1 张${keepKindText}，弃置其余 ${discardCount} 张`
    );
  }
...
function publicEnvironment(env) {
  ...
  return {
    ...
    stashCount:
      !env.stashClaimed && Array.isArray(env.stashCards)
...
function canPlayerAct(game, player) {
  ...
  if (
    game.pendingRobberyPick &&
    game.pendingRobberyPick.targetId === player.id
  ) {
    return true;
  }
  if (game.phase === 'settle_act') {
...
function getActingPlayerIds(game) {
  ...
  if (game.pendingRobberyPick && game.pendingRobberyPick.targetId) {
    return [game.pendingRobberyPick.targetId];
  }
  if (game.phase === 'init_roll') {
...
function forceTimeout(game, playerId) {
  ...
  if (
    game.pendingRobberyPick &&
    game.pendingRobberyPick.targetId === playerId
  ) {
...
function onPlayerQuit(game, playerId) {
  ...
  resolveBoostedTycoon(game);
  if (game.workshopMasterPlayerId === playerId) {
