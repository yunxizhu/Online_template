...
window.LasidaoFx = (function () {
  ...
  async function playSlot(game, slot) {
    ...
    if (slot.envReward && (slot.envReward.rewards || []).length) {
      await playEnvReward({ game, ...slot.envReward });
    } else if (slot.envReveal) {
      await playEnvReveal({ game, ...slot.envReveal });
    }
  ...
  function rewardFlyCount(reward) {
    const total = Number(reward && reward.total) || 0;
    if (total > 0) return total;
    return (reward && reward.detail ? reward.detail : []).reduce(
      (n, d) => n + (Number(d && d.amount) || 0),
      0
    );
  }

  function slotStashCardEls(number) {
    if (number == null) return [];
    return Array.from(
      document.querySelectorAll(
        `.las-slot[data-area="resource"][data-num="${number}"] .las-env-stash-card`
      )
    );
  }

  function envBoardTileEl(envId, number) {
    let srcEl = envId ? tileEl(envId) : null;
    if (!srcEl && number) {
  ...
  async function flyEnvSideCard(opts) {
    ...
    const gen = settleGen;
    const layer = ensureLayer();
    if (!layer || !opts.claimPid) return;
    throwIfSettleAborted(gen);
    const number = opts.number;
    const srcEl = document.querySelector(
      `.las-slot[data-area="resource"][data-num="${number}"] .las-env-side-card`
    );
    const target = playerEl(opts.claimPid);
    if (!srcEl || !target) return;
    const kind = opts.sideCardKind === 'building' ? 'building' : 'function';
    await flySettleCard(
      layer,
      {
        id: (opts.envId || 'env') + ':side',
        kind,
        faceDown: true,
      },
      kind,
  ...
  async function presentScoreAwards(awards) {
    const gen = settleGen;
    const layer = ensureLayer();
    if (!layer || !awards || !awards.length) return;
    for (const a of awards) {
      throwIfSettleAborted(gen);
      const target = playerEl(a.pid);
      const from = rectCenter(target);
      if (!from) continue;
      const pop = document.createElement('div');
      pop.className = 'las-fx-pop is-win';
      pop.textContent = '+' + (Number(a.amount) || 1);
      pop.style.left = from.x + 'px';
      pop.style.top = from.y - 18 + 'px';
      layer.appendChild(pop);
      const scoreEl = target.querySelector('.las-me-info-score');
      if (scoreEl) {
        scoreEl.classList.remove('is-gain-pop');
        void scoreEl.offsetWidth;
        scoreEl.classList.add('is-gain-pop');
        setTimeout(() => scoreEl.classList.remove('is-gain-pop'), 750);
      }
      await sleep(700);
      throwIfSettleAborted(gen);
  ...
  async function playEnvReveal(opts) {
    opts = opts || {};
    await presentEnvCard(opts);
    if (opts.claimPid) {
      await flyEnvSideCard(opts);
    }
    if (opts.scoreAwards && opts.scoreAwards.length) {
      await presentScoreAwards(opts.scoreAwards);
    }
  }
  ...
  async function playEnvReward(opts) {
    ...
    const srcCenter = rectCenter(envBoardTileEl(opts.envId, number));
    const stashEls = slotStashCardEls(number);
    const stashFrom = stashEls.map((el) => rectCenter(el)).filter(Boolean);

    await presentEnvCard(opts);
    throwIfSettleAborted(gen);
    if (!rewards.length) return;

    const stashWrap = document.querySelector(
      `.las-slot[data-area="resource"][data-num="${number}"] .las-env-stash-wrap`
    );

    // 2. 从事件旁暗置资源卡背飞向目标玩家版面
    const fallbackFrom =
      srcCenter || rectCenter($('las-board-resource'));
    if (!fallbackFrom && !stashFrom.length) return;

    for (const reward of rewards) {
      throwIfSettleAborted(gen);
      const pid = reward.pid;
      const count = rewardFlyCount(reward);
      if (count <= 0) continue;
      ...
      const flies = [];
      for (let i = 0; i < count; i++) {
        const from = stashFrom[i] || stashFrom[stashFrom.length - 1] || fallbackFrom;
        const fly = document.createElement('div');
      ...
      if (stashWrap) stashWrap.classList.add('is-settle-claimed');
      for (const el of stashEls) el.classList.add('is-settle-claimed');

      const jobs = [];
      for (let i = 0; i < flies.length; i++) {
        const { fly, from } = flies[i];
        jobs.push(
          (async () => {
            await sleep(i * 100);
            const jitter = {
      ...
      throwIfSettleAborted(gen);
      const Ui = window.LasidaoUi;
      if (Ui && typeof Ui.popResourceBadges === 'function') {
        Ui.popResourceBadges([reward]);
      }
      await sleep(150);
    }
  }
