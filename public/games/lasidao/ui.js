...
window.LasidaoUi = (function () {
  ...
  /** 投掷动画卡在 rolling/grouping 时强制进入 ready，避免「看见结果却点不了板块」 */
  let diceReadyWatchdog = null;
  ...
  /** 'wishWell' | null — 与丰收/事件选资源共用弹框时的提交方式 */
  ...
  let buildRecycleKey = null;
  let buildRecyclePlaying = false;
  ...
  function resetDiceAnim() {
    ...
    diceAnim.stage = 'idle';
    diceAnim.revealed = false;
    diceAnim.finalDice = [];
    diceAnim.finalBoosted = [];
    resetDiceSelection();
  }

  /** 投掷动画结束：进入 ready 并立刻刷新板块可点状态（避免只更新骰区、槽位仍 disabled） */
  function markProduceDiceReady(meId, opts) {
    const o = opts || {};
    if (diceReadyWatchdog) {
      clearTimeout(diceReadyWatchdog);
      diceReadyWatchdog = null;
    ...
    diceAnim.revealed = true;
    if (Array.isArray(o.finalDice)) diceAnim.finalDice = o.finalDice.slice();
    if (Array.isArray(o.finalBoosted)) {
      diceAnim.finalBoosted = o.finalBoosted.slice();
    }
    const wrap = $('las-dice-wrap');
    if (wrap) wrap.hidden = false;
    if (o.renderGrouped !== false) {
      if (o.remote) renderRemoteDice(lastGame, meId || lastMeId);
      else if (o.spectator) {
        renderSpectatorDice(
          diceAnim.finalDice,
          o.color,
    ...
    updateDiceHint();
    const boardMe = meId || lastMeId;
    if (lastGame && boardMe) renderBoard(lastGame, boardMe);
    flushDeferredHeavyRender();
  }

  function armDiceReadyWatchdog(meId) {
    if (diceReadyWatchdog) clearTimeout(diceReadyWatchdog);
    diceReadyWatchdog = setTimeout(() => {
      diceReadyWatchdog = null;
      if (diceAnim.stage !== 'rolling' && diceAnim.stage !== 'grouping') {
        return;
      }
      console.warn('[lasidao] diceAnim watchdog: force ready');
      markProduceDiceReady(meId || lastMeId);
    }, 3200);
  }
  ...
  function isSettleDeferredObtainReveal(reveal, game) {
    if (!reveal || !game || game.phase !== 'settle') return false;
    const card = reveal.card;
    return Boolean(card && (card.instantScore || card.buildType === 'score1'));
  }
  ...
  function maybeShowPlayRevealFromState(game, prev, meId) {
    ...
    if (isSettleDeferredObtainReveal(reveal, game)) return false;
    return showPlayReveal(reveal, meId);
  }

  function onPlayReveal(data) {
    const reveal = data && data.reveal;
    if (isSettleDeferredObtainReveal(reveal, lastGame)) return;
    showPlayReveal(reveal, lastMeId);
  }

  function showSettleObtain(reveal) {
    if (!reveal) return false;
    const payload = {
      ...reveal,
      id: reveal.id || 'settle-obtain-' + Date.now(),
      forceAll: true,
    };
    return showPlayReveal(payload, lastMeId);
  }
  ...
  function captureTurnToastSnap(game, meId) {
    const me = mePlayer(game, meId);
    const idle = me == null ? 0 : playerIdleCount(me);
    return {
      phase: game.phase,
      round: game.round,
  ...
  function ensureModalVisToggleBtn(modal) {
    ...
    btn.addEventListener('click', (e) => {
      ...
      btn.classList.toggle('is-blink', collapsed);
  ...
  function appendWorkerDice(container, face, workers, players, game, opts) {
    ...
    const previewMergedIntoExisting = Boolean(
      previewPid &&
        previewCount > 0 &&
        entries.some(([pid]) => pid === previewPid)
    );

    for (const [pid, n] of entries) {
      const color = playerDieColor(players, pid, game);
      let count = Math.min(Number(n) || 0, 24);
      let boosted = Math.min(Number(boosts[pid]) || 0, count);
      const isPreviewMerge =
        previewMergedIntoExisting && pid === previewPid && previewCount > 0;
      if (isPreviewMerge) {
        count = Math.min(count + previewCount, 24);
        boosted = Math.min(boosted + previewBoostAdd, count);
      }
      ...
      row.appendChild(mul);
      if (isPreviewMerge) {
        row.title = t('lasidao.dispatchPreviewMerged', {
    ...
    if (opts.overlay) finalizeOverlayWorkerDice(container);
    return wrap;
  }

  const SLOT_DIE_SCALE_MIN = 0.32;
  const slotDieFitObservers = typeof WeakMap !== 'undefined' ? new WeakMap() : null;

  function overlayDiceHost(containerEl) {
    if (!containerEl) return null;
    if (
      containerEl.classList &&
      (containerEl.classList.contains('las-slot-overlay') ||
        containerEl.classList.contains('las-personal-build-num'))
    ) {
      return containerEl;
    }
    return containerEl.closest
      ? containerEl.closest('.las-slot-overlay, .las-personal-build-num')
      : null;
  }

  function setSlotDieScale(containerEl, scale) {
    if (!containerEl) return;
    const s = Math.max(SLOT_DIE_SCALE_MIN, Math.min(1, scale));
    containerEl.style.setProperty(
      '--las-slot-die-scale',
      String(Number(s.toFixed(3)))
    );
  }

  function fitSlotWorkerDice(containerEl) {
    if (!containerEl) return;
    const wrap = containerEl.querySelector('.las-worker-dice.is-overlay');
    if (!wrap) {
      containerEl.style.removeProperty('--las-slot-die-scale');
      return;
    }
  ...
  function observeSlotWorkerDiceFit(containerEl) {
    if (!containerEl || typeof ResizeObserver === 'undefined' || !slotDieFitObservers) {
      return;
    }
    if (!slotDieFitObservers.has(containerEl)) {
      const obs = new ResizeObserver(() => fitSlotWorkerDice(containerEl));
      slotDieFitObservers.set(containerEl, obs);
      obs.observe(containerEl);
    }
  }

  function finalizeOverlayWorkerDice(containerEl) {
    const host = overlayDiceHost(containerEl);
    if (!host) return;
    requestAnimationFrame(() => {
      fitSlotWorkerDice(host);
      observeSlotWorkerDiceFit(host);
    });
  }

  function fitAllBoardSlotOverlayDice() {
    for (const areaKey of ['resource', 'special']) {
      const boardEl = $('las-board-' + areaKey);
      if (!boardEl) continue;
      boardEl.querySelectorAll('.las-slot-overlay').forEach((el) => {
        fitSlotWorkerDice(el);
        observeSlotWorkerDiceFit(el);
      });
    }
  }
  ...
  function produceManagerBuildLabel(title) {
    const label = title && title.label ? String(title.label) : '';
    if (label.endsWith('管理者')) {
      return `${label.slice(0, -3)}建筑`;
    }
    const buildLabel = title && title.buildLabel ? String(title.buildLabel) : '';
    if (buildLabel) {
      return buildLabel.replace(/·[^·]+$/, '') || buildLabel;
    }
    return label;
  }

  function lasTitleHoverLines(title) {
    if (!title) return [];
    if (title.id === 'commerceTycoon' || title.stackKey === 'exchange') {
      return [t('lasidao.titleCommerceTycoonTip')];
    }
    ...
    if (title.id === 'whatYouWant' || title.stackKey === 'whatYouWant') {
      return [t('lasidao.titleWhatYouWantTip', { need, score })];
    }
    ...
    return [
      t('lasidao.titlePoolTip', {
        need,
        score,
        build: title.buildLabel || title.label || '',
      }),
    ];
  }

  function lasTitleBadgeTip(title) {
    const lines = lasTitleHoverLines(title);
    return lines.join('\n');
  }

  function renderTitlePool(game) {
    const root = $('las-title-pool');
    if (!root) return;
    root.hidden = false;
    const titles = (game && game.availableTitles) || [];
    root.innerHTML = '';
    const head = document.createElement('div');
    head.className = 'las-title-pool-label muted';
    head.textContent = t('lasidao.titlePoolLabel');
    root.appendChild(head);
    const list = document.createElement('div');
    list.className = 'las-title-pool-list';
    if (!titles.length) {
      const empty = document.createElement('span');
      empty.className = 'las-title-pool-empty muted';
      empty.textContent = t('lasidao.titlePoolEmpty');
      list.appendChild(empty);
    } else {
      for (const title of titles) {
        const item = document.createElement('span');
        item.className = 'las-title-pool-item';
        item.tabIndex = 0;
        const label = document.createElement('span');
        label.className = 'las-title-pool-item-text';
        label.textContent = title.label || title.buildLabel || '';
        item.appendChild(label);
        const tip = document.createElement('span');
        tip.className = 'las-title-pool-tip';
        tip.setAttribute('role', 'tooltip');
        const lines = lasTitleHoverLines(title);
        lines.forEach((line, idx) => {
          const row = document.createElement('span');
          row.className =
            'las-title-pool-tip-line' + (idx > 0 ? ' muted' : '');
  ...
  function isPlayerParallelPhasePending(game, p, meId) {
    ...
    if (game.phase === 'build') {
      return Boolean(p.needsDiscardBuild);
    }
  ...
  function selfEventChoiceStatus(choice) {
    ...
    switch (choice.needChoice) {
      ...
      case 'moveNeutral':
        return t('lasidao.statusEventMoveNeutral', { name, need });
      case 'recallDie':
  ...
  function renderLasStatus(game, meId) {
    ...
    if (!game.over) {
      if (game.pendingTrade) {
        ...
        const overflowP = (game.players || []).find((p) => p.needsDiscardBuild);
        if (overflowP) {
          const demolition =
            overflowP.pendingDiscardBuildSource === 'demolition' ||
            (overflowP.pendingDiscardBuild &&
              overflowP.pendingDiscardBuild.source === 'demolition');
          if (overflowP.id === meId) {
            text = demolition
  ...
  function updateProduceRollButtonLabel(game, meId) {
    ...
    if (game && isMercenaryRollMode(game, meId)) {
      ...
      if (autoProduceRollEndsAt > 0 && remaining > 0) {
        ...
        rollBtn.textContent = t('lasidao.eventMercenaryRollCountdown', { n: secs });
      } else {
  ...
  /** 含 lastPlacerId：同轮再次轮到自己时 key 必须变，否则会吞掉自动投/投掷区逻辑 */
  function produceAwaitRollKey(game, meId) {
    if (!game || !meId) return null;
    return `${game.round || 0}:${meId}:${game.currentPlayerId || ''}:${game.lastPlacerId || ''}:await`;
  }

  function syncAutoProduceRoll(game, meId) {
    const kind = autoRollKind(game, meId);
    const key =
      kind === 'produce'
        ? produceAwaitRollKey(game, meId)
        : kind === 'mercenary'
          ? `merc:${game.round || 0}:${meId}:${game.currentPlayerId || ''}:${game.lastPlacerId || ''}`
          : null;
    if (!kind) {
      clearAutoProduceRoll();
      // 仅在「确有骰面」或已离开生产时清空 sentKey，允许同回合再次轮到时自动投。
      // pulse 先清 awaiting、骰子尚未到达时绝不能清，否则迟到的 awaiting 快照会二次自动投。
      if (
        !game ||
        game.phase !== 'produce' ||
  ...
  function diceReady() {
    ...
    if (lastGame && isMercenaryPlaceMode(lastGame, lastMeId)) {
      if (diceAnim.stage === 'ready' || diceAnim.stage === 'grouping') return true;
      if (diceAnim.stage === 'rolling' && diceAnim.revealed) return true;
      return Boolean(diceAnim.finalDice && diceAnim.finalDice.length);
    }
    // 生产派遣：必须有服务端确认的骰面。禁止仅凭旁观残留动画解锁（加入端常见：看见旧 2/5 却无法真正派遣）
    if (
      !lastGame ||
    ...
    const serverDice = Array.isArray(lastGame.dice) ? lastGame.dice : [];
    if (!serverDice.length) return false;
    if (diceAnim.stage === 'ready' || diceAnim.stage === 'grouping') return true;
    if (diceAnim.stage === 'rolling' && diceAnim.revealed) return true;
    // 动画卡死时仍允许：服务端已有骰
    return true;
  ...
  /** 可用点数：生产阶段始终以服务端 dice 为准，动画只负责展示 */
  function availableFaces() {
    ...
    const counts = countByFace(source);
  ...
  function produceFuncConfirmActive(game, meId) {
    if (!game || game.phase !== 'produce' || !selectedFuncId || !meId) {
      return false;
    }
    if (!isMyTurn(game, meId)) return false;
    const me = mePlayer(game, meId);
    const card =
      me && (me.funcCards || []).find((c) => c.id === selectedFuncId);
    return Boolean(
      card && canPlayFuncCard(game, meId, card.funcType)
    );
  }
  ...
  function updateDispatchPreview() {
    ...
    if (produceFuncConfirmActive(lastGame, lastMeId)) {
      const wrap = $('las-dice-wrap');
      ...
      confirm.textContent = t('lasidao.confirmUse');
      syncSlotConfirmButtons();
      syncSlotDispatchPreview();
      return;
    }

    if (lastGame && lastGame.phase === 'produce') {
  ...
  function updateDiceHint() {
    ...
    if (
      produceFuncConfirmActive(lastGame, lastMeId)
    ) {
      const me = mePlayer(lastGame, lastMeId);
      const card =
        me && (me.funcCards || []).find((c) => c.id === selectedFuncId);
      hint.textContent =
        (card && (funcRuleText(card.funcType) || card.label)) ||
        t('lasidao.confirmUse');
      return;
    }
  ...
  function replaceSlotOverlayDice(overlayEl, face, workers, boosts, game, previewAdd) {
    ...
    finalizeOverlayWorkerDice(overlayEl);
    return diceRow;
  }
  ...
  function pendingMoveNeutralCount(choice) {
    return Math.max(1, Number(choice && choice.count) || 1);
  }
  ...
  /** 召回可选骰：派遣目标格扣除刚放置后的旧骰 */
  function slotRecallableDieCounts(game, areaKey, num, pid) {
    const counts = slotPlayerDieCounts(game, areaKey, num, pid);
    const c = game && game.pendingEventChoice;
    if (!c || c.needChoice !== 'recallDie') {
      return counts;
    }
    const exArea = c.excludeArea || 'resource';
    const exNum = Number(c.excludeNumber != null ? c.excludeNumber : c.number);
    if (!(areaKey === exArea && num === exNum)) {
      return counts;
    }
    const justPlaced = Math.max(0, Number(c.justPlacedCount) || 0);
    const justBoosted = Math.min(
      justPlaced,
      Math.max(0, Number(c.justPlacedEnhanced) || 0)
    );
  ...
  function syncRecallTeleportSlotPopovers(game, meId) {
    if (
      ...
      const counts = slotRecallableDieCounts(
        game,
        recallPickArea,
        recallPickNumber,
        meId
    ...
    if (
      ...
      mountSlotPickPopover(teleportPickArea, teleportPickNumber, (pop) => {
        if (teleportPickTargetId) {
          ...
          const hintColor = playerDieColor(
            game.players,
            teleportPickTargetId,
            game
          );
          const hintSwatch = document.createElement('span');
          hintSwatch.className = 'las-die-swatch color-' + hintColor;
          hintSwatch.setAttribute('aria-hidden', 'true');
          playerHint.appendChild(hintSwatch);
          playerHint.appendChild(
            document.createTextNode(
              workerName(teleportPickTargetId, game.players, game)
            )
          );
        ...
        for (const [pid, c] of occ) {
          grid.appendChild(
            makeExilePlayerButton(game, pid, c, () =>
              onTeleportPlayerSelected(game, pid)
            )
          );
        }
        pop.appendChild(grid);
  ...
  function recallSlotSelectable(game, meId, areaKey, num, workers) {
    ...
    return slotRecallableDieCounts(game, areaKey, num, meId).total > 0;
  }
  ...
  function maybeShowNeutralToast(game, meId) {
    ...
    showTurnToast(
      t('lasidao.eventMoveNeutralToast', {
        need: pendingMoveNeutralCount(c),
      }),
      2200
    );
  }
  ...
  function confirmNeutralPlace() {
    ...
    const moveCount = pendingMoveNeutralCount(lastGame.pendingEventChoice);
  ...
  function syncNeutralPickUi(game, meId) {
    ...
    const need = pendingMoveNeutralCount(game.pendingEventChoice);
    if (hint) {
      hint.textContent =
        selectedTarget && selectedTarget.type === 'area'
          ...
          : t('lasidao.eventMoveNeutralHint', { need });
    }
    if (preview) preview.hidden = true;
    if (voidBtn) voidBtn.hidden = true;
    if (produceActions) produceActions.hidden = false;
  ...
  function applyBoardSlotInteraction(slot, ix) {
    ...
    // 绝不用 native disabled：投掷动画期间会把槽位 disabled，若 ready 回调丢失，
    // 浏览器会直接吞掉 click，表现为「看得见骰面却无法点板块派遣」，只能刷新。
    slot.disabled = false;
    slot.classList.toggle('is-inert-slot', !slotSelectable);
    slot.setAttribute('aria-disabled', slotSelectable ? 'false' : 'true');
    if (slotSelectable) {
  ...
  function handleBoardSlotClick(areaKey, num) {
    ...
    if (ix.recallSelectable) {
      if (!netRef) return;
      const counts = slotRecallableDieCounts(game, areaKey, num, meId);
      if (counts.normal > 0 && counts.enhanced > 0) {
  ...
  function ensureBoardClickDelegate(boardEl, areaKey) {
    ...
    boardEl.addEventListener('dblclick', (ev) => {
      ...
      if (!Number.isFinite(num)) return;
      handleBoardSlotClick(areaKey, num);
      if (lastGame && lastGame.phase === 'produce' && isMyTurn(lastGame, lastMeId)) {
        confirmDispatch();
      }
    });
  }
  ...
  function renderAreaBoard(game, meId, areaKey) {
    ...
    syncBoardPickHighlight();
    requestAnimationFrame(fitAllBoardSlotOverlayDice);
  ...
  function startDiceAnimation(finalDice, meId, boostFlags) {
    ...
    armDiceReadyWatchdog(meId);

    const wrap = $('las-dice-wrap');
    const diceEl = $('las-dice');
    const groupsEl = $('las-dice-groups');
    if (!wrap || !diceEl || !groupsEl) {
      markProduceDiceReady(meId);
      return;
    }
    ...
    const t1 = setTimeout(() => {
      ...
      diceAnim.revealed = true;
      updateDiceHint();
      // 亮面即可点板块，不必等 grouping/ready；并立刻刷掉动画期留下的 inert
      if (lastGame && meId) renderBoard(lastGame, meId);
    }, 900);
  ...
  function setDiceTitle(_text) {
    /* 骰子区标题已移除，保留空函数避免调用处改动 */
  }
  ...
  function isPostProduceRetentionPhase(phase) {
    return (
      phase === 'settle' ||
      phase === 'settle_act' ||
      phase === 'wish_well' ||
  ...
  function renderPostProduceDiceRetain(game, meId) {
    const wrap = $('las-dice-wrap');
    const confirm = $('btn-las-confirm');
    const voidBtn = $('btn-las-void');
    const preview = $('las-dispatch-preview');
    ...
    if (diceAnim.retainKey !== retainKey || diceAnim.stage === 'idle') {
      diceAnim.retainKey = retainKey;
  ...
  function boardHasAnyTiles(game) {
    if (!game || !game.board) return false;
    for (const area of ['resource', 'special']) {
      const tiles = game.board[area] && game.board[area].tiles;
      if (Array.isArray(tiles) && tiles.length) return true;
    }
    return false;
  }

  function maybePlayBuildRecycle(game, prevGame) {
    if (!game || game.phase !== 'build' || !prevGame || buildRecyclePlaying) return;
    const key = (game.round || 0) + ':build';
    if (buildRecycleKey === key) return;
    const hadBoard =
      boardHasAnyTiles(prevGame) ||
      boardHasAnyWorkers(prevGame) ||
      (prevGame.dice || []).length > 0;
    buildRecycleKey = key;
    if (!hadBoard) return;

    buildRecyclePlaying = true;
    renderBoard(prevGame, lastMeId);
    renderPostProduceDiceRetain(prevGame, lastMeId);
    const LasFx = window.LasidaoFx;
    const run =
      LasFx && typeof LasFx.playRecycleBoard === 'function'
        ? () => LasFx.playRecycleBoard(prevGame)
        : () => Promise.resolve();
    Promise.resolve(run())
      .catch(() => {})
      .finally(() => {
        buildRecyclePlaying = false;
        if (lastGame) {
          renderBoard(lastGame, lastMeId);
  ...
  function renderDice(game, meId, _prevGame) {
    ...
    if (isPostProduceRetentionPhase(game.phase)) {
      renderPostProduceDiceRetain(game, meId);
      return;
    }
    ...
    // 本人回合：服务端骰面与本地动画不一致时，丢弃残留（旁观/失败清空后的脏状态）
    if (
      myTurn &&
      dice.length &&
      (diceAnim.finalDice || []).length &&
      dice.join(',') !== (diceAnim.finalDice || []).join(',') &&
      diceAnim.stage !== 'rolling' &&
      diceAnim.stage !== 'grouping'
    ) {
      lastAnimatedRollKey = null;
    ...
    if (!dice.length) {
      if (diceAnim.stage !== 'idle') resetDiceAnim();
      if (myTurn && produceFuncConfirmActive(game, meId)) {
        wrap.hidden = false;
        if (produceActions) produceActions.hidden = false;
        const diceEl = $('las-dice');
        const groupsEl = $('las-dice-groups');
        ...
        if (interactive) {
          ...
          if (discardMode) {
            ...
            btn.ondblclick = (e) => {
              ...
              if (!playable || isLasCardInert(btn)) return;
              if (!netRef) return;
              netRef.sendAction('useFunc', { cardId: c.id });
              selectedFuncId = null;
              resetDiceSelection();
              syncBoardPickHighlight();
              renderPlayerBoards(game, meId);
  ...
  function appendBuildDiscardChoiceUi(parent, game, meId, me) {
    ...
    tip.textContent = demolition
      ? unbuiltExisting.length
        ? t('lasidao.discardBuildDemolitionTip', {
            label: pending.newCard.label || '?',
          })
        : t('lasidao.discardBuildDemolitionOnlyTip', {
            label: pending.newCard.label || '?',
          })
      : unbuiltExisting.length
    ...
    newLab.textContent = demolition
      ? t('lasidao.pendingDemolishBuild')
      : t('lasidao.pendingNewBuild');
  ...
  function beginSelectedFuncUse(game, me) {
    ...
    if (card.funcType === 'harvest') {
      ...
      netRef.sendAction('useFunc', { cardId: card.id });
    } else if (card.funcType === 'shelter') {
      netRef.sendAction('useFunc', { cardId: card.id });
    } else if (card.funcType === 'welfareHouse') {
      netRef.sendAction('useFunc', { cardId: card.id });
  ...
  function makeBoardBuildingCard(game, meId, p, b, isMe, reveal) {
    ...
    if (isMe) {
      ...
      btn.ondblclick = (e) => {
        e.stopPropagation();
        if (isLasCardInert(btn)) return;
        if (game.phase === 'build' && isMyTurn(game, meId) && !p.buildPassed && !b.built) {
          if (canPay(p.resources || {}, b.cost || {})) {
            if (!netRef) return;
            selectedFuncId = null;
            selectedPermanent = null;
      ...
      if (
        ...
        if (canPay(p.resources || {}, b.cost || {})) {
          btn.classList.add('is-affordable');
        }
      }
    } else {
      setLasCardInert(btn, true);
  ...
  function appendPlayerTitleBadges(parent, player) {
    ...
    for (const title of titles) {
      ...
      badge.title = lasTitleBadgeTip(title);
  ...
  function syncWishWellModal(game, meId) {
    ...
    const shouldOpen =
      ...
      !settlePlaying &&
      game.phase !== 'settle';
    // 旧许愿井按钮式弹框不再使用，统一走丰收样式选资源框
    setWishWellModalOpen(false);

    if (shouldOpen) {
      const harvestModal = $('las-harvest-modal');
      const alreadyOpen =
        harvestModal &&
        !harvestModal.hidden &&
        harvestAction === 'wishWell' &&
        harvestMaxCount === need;
      if (!alreadyOpen) {
        openHarvestModal({
  ...
  function getSettleDiscardKind(game, me) {
    ...
    if (game.phase === 'event_discard') {
      return Number(game.pendingPrisonerDiscard) > 0 ? 'prisoner' : null;
    }
    if (game.phase !== 'settle_act') return null;
  ...
  function settleDiscardResNeed(game, me) {
    if (!me) return 0;
    if (game && game.phase === 'event_discard') {
      const left = Number(game.pendingPrisonerDiscard) || 0;
      const total = Object.values(me.resources || {}).reduce(
        (a, b) => a + (Number(b) || 0),
        0
      );
      return Math.min(left, total);
    }
    if (!me.pendingDiscardRes) return 0;
    const max =
      game.me && game.me.maxResourceHand != null
        ? game.me.maxResourceHand
        : me.maxResourceHand != null
  ...
  function setSettleDiscardSource(kind) {
    const sourceEl = $('las-settle-discard-source');
    if (!sourceEl) return;
    if (kind === 'prisoner') {
      sourceEl.textContent = t('lasidao.discardSourcePrisoner');
      sourceEl.hidden = false;
    } else if (kind === 'res' || kind === 'func' || kind === 'build') {
      sourceEl.textContent = t('lasidao.discardSourceSettle');
      sourceEl.hidden = false;
    } else {
  ...
  function updateSettleDiscardModalUi(game, me, kind) {
    ...
    if (kind === 'res' || kind === 'prisoner') {
      const need = settleDiscardResNeed(game, me);
      const picked = discardResPickTotal();
      if (totalEl) {
        if (kind === 'prisoner') {
          totalEl.textContent = t('lasidao.discardPrisonerTip', {
            need,
            picked,
          });
        } else {
  ...
  function renderSettleDiscardResBody(body, game, me, kind) {
    const labels = getResLabels(game);
    const Assets = window.LasidaoAssets;
    const resKind = kind || settleDiscardModalKind || 'res';
    const need = settleDiscardResNeed(game, me);
    ...
    for (const res of RESOURCES) {
      ...
      minus.onclick = () => {
        if ((discardResPick[res] || 0) > 0) {
          discardResPick[res] = (discardResPick[res] || 0) - 1;
          updateSettleDiscardModalUi(game, me, resKind);
        }
      };
  ...
  function renderSettleDiscardModal(game, me) {
    ...
    body.innerHTML = '';
    if (!kind) return;
    setSettleDiscardSource(kind);

    if (kind === 'res' || kind === 'prisoner') {
      const need = settleDiscardResNeed(game, me);
      if (kind === 'prisoner' && need <= 0) {
        if (Number(game.pendingPrisonerDiscard) > 0 && netRef) {
      ...
      titleEl.textContent =
        kind === 'prisoner'
          ? t('lasidao.eventPrisonerHint', { n: need })
          : t('lasidao.statusSettleActYouRes');
      renderSettleDiscardResBody(body, game, me, kind);
    } else if (kind === 'func') {
  ...
  function renderFuncForm(game, me) {
    ...
    if (
      ...
      me &&
      selectedFuncId &&
      !canPlayFuncCard(game, me.id, ((me.funcCards || []).find((c) => c.id === selectedFuncId) || {}).funcType)
    ) {
      selectedFuncId = null;
    }
    if (game && game.phase === 'produce') {
  ...
  function syncEventUi(game, meId) {
    ...
    if (!showChoice) {
      ...
      if (harvestSourceText && harvestAction !== 'wishWell') {
        setHarvestModalOpen(false);
      }
      return;
    }
  ...
  function render(game, net, opts) {
    ...
    const willPlaySettle = (() => {
      ...
      if (!shouldPlaySettleAnim(game.phase)) return false;
      const key = (report.round || '') + ':' + report.at;
      return settleAnimKey !== key && !settlePlaying;
    })();
    ...
    // 投掷动画中也要跑 renderDice：否则 pulse/迟到 state 无法落地，会出现「骰面在播但板块永远不可点」
    renderDice(game, meId, _prevGame);
    if (!deferHeavy) {
      renderProduceIdleBar(game);
    }
    renderTitlePool(game);
    ...
    maybePlayBuildRecycle(game, _prevGame);
    ...
    if (permAct) {
      ...
      if (me) {
        ...
        if (resetBuildBtn) {
          const usedBuyFunc = Boolean(
            (me && me.turnUsedBuyFunc) ||
              (game.me && game.me.turnUsedBuyFunc)
          );
          const usedRedraw = Boolean(
            (me && me.turnUsedRedraw) ||
              (game.me && game.me.turnUsedRedraw)
          );
          const ok =
            myBuildTurn && !usedBuyFunc && !usedRedraw && !mustDiscard;
  ...
  /** 仅结算阶段播放结算动画；已到弃牌及之后（含建造）刷新重连不再重播 */
  function shouldPlaySettleAnim(phase) {
    return phase === 'settle';
  }
      ...
      if (Array.isArray(slot.tiles) && slot.tiles.length) {
        ...
        for (const t of slot.tiles) {
          ...
          const shown = {
            ...t,
            number: num,
            faceDown: Boolean(t.wasFaceDown || t.faceDown),
          };
          if (idx >= 0) {
            tiles[idx] = { ...tiles[idx], ...shown, faceDown: tiles[idx].faceDown };
          } else {
            tiles.push(shown);
            ids.add(t.id);
          }
  ...
  function buildSettleBoardFreeze(game, prevGame) {
    ...
    for (const slot of report.slots || []) {
      ...
      for (const rt of slot.tiles || []) {
        ...
        if (idx >= 0) {
          // 保留结算前朝向，获得后亮出由结算动画负责
          tiles[idx] = { ...tiles[idx], number: num };
        }
      }
      ...
      if (Array.isArray(slotArr)) {
        ...
        if (s) {
          ...
          for (const rt of slot.tiles || []) {
            ...
            if (idx2 >= 0) {
              stiles[idx2] = { ...stiles[idx2], number: num };
            }
          }
          s.tiles = stiles;
        }
  ...
  function maybePlaySettle(game) {
    ...
    if (!shouldPlaySettleAnim(game.phase)) {
      return;
    }
    ...
    const run = async () => {
      try {
        ...
        if (!settlePlaying || settleAnimKey !== key) return;
    ...
    Promise.resolve(run())
      .then(() => {
        if (settlePlaying && settleAnimKey === key) finishSettleUi(key);
      })
      .catch(() => {
        if (settlePlaying && settleAnimKey === key) finishSettleUi(key);
      });
  ...
  async function confirmDispatch() {
    ...
    if (!netRef) {
      showTurnToast(t('toast.opFail'), 1800);
      return;
    }
    if (dispatchBusy) return;
    if (!selectedTarget || selectedFace == null) {
      showTurnToast(t('lasidao.pickTargetFirst'), 1800);
      return;
    }

    // 服务端仍在等待投掷：先补投，再让玩家派遣（加入端隧道丢包时常见）
    if (
      lastGame &&
      lastGame.phase === 'produce' &&
      isMyTurn(lastGame, lastMeId) &&
      ...
      showTurnToast(t('lasidao.produceRoll'), 1600);
      netRef.sendAction('produceRoll', {});
      return;
    }

    const serverDice = (lastGame && lastGame.dice) || [];
    const remote = lastGame && isRemoteMode(lastGame);
    if (remote) {
      if (!selectedWildCount || selectedFace == null) {
        showTurnToast(t('lasidao.pickTargetFirst'), 1800);
        return;
      }
      const payload = {
      ...
      // 等服务端 state 再清骰，避免失败时本地被掏空却点不动
      return;
    }

    if (serverDice.indexOf(Number(selectedFace)) < 0) {
      showTurnToast(t('lasidao.noSuchDie'), 2200);
      if (lastGame && lastMeId) {
        diceAnim.finalDice = serverDice.slice();
        diceAnim.finalBoosted = Array.isArray(lastGame.diceBoosted)
          ? lastGame.diceBoosted.slice()
          : [];
        diceAnim.stage = 'ready';
  ...
  function onGameError(data) {
    ...
    if (
      ...
      lastGame.phase === 'produce' &&
      isMyTurn(lastGame, lastMeId) &&
      (/投掷|遥控/.test(msg) || isAwaitingRoll(lastGame))
    ) {
      if (isAwaitingRoll(lastGame) || !(lastGame.dice && lastGame.dice.length)) {
        netRef.sendAction('produceRoll', {});
      }
    }
    // 恢复骰区，避免乐观清空后按钮点了没反应
    if (lastGame && lastMeId && lastGame.phase === 'produce') {
      const dice = Array.isArray(lastGame.dice) ? lastGame.dice : [];
      if (dice.length && !isAwaitingRoll(lastGame)) {
        diceAnim.finalDice = dice.slice();
        diceAnim.finalBoosted = Array.isArray(lastGame.diceBoosted)
          ? lastGame.diceBoosted.slice()
          : [];
  ...
  function setIllegalBuildModalOpen(open) {
    ...
    if (!open) {
      ...
      illegalBuildSelectedId = null;
      const confirmBtn = $('btn-las-illegal-build-confirm');
      if (confirmBtn) {
        confirmBtn.hidden = true;
        confirmBtn.disabled = true;
      }
      const hint = $('las-illegal-build-hint');
      if (hint) {
        hint.hidden = true;
        hint.textContent = '';
      }
  ...
  function syncIllegalBuildFoot(mode, pending) {
    ...
    if (cancelBtn) {
      ...
      if (showCancel) cancelBtn.textContent = t('lasidao.cancel');
    }
    if (confirmBtn) {
      const showConfirm = mode === 'building';
      confirmBtn.hidden = !showConfirm;
      if (showConfirm) {
        confirmBtn.textContent = t('lasidao.illegalBuildConfirm');
        confirmBtn.disabled = !illegalBuildSelectedId;
      } else {
        confirmBtn.disabled = true;
      }
    }
  ...
  function updateIllegalBuildHint(builtList) {
    const hint = $('las-illegal-build-hint');
    if (!hint) return;
    hint.hidden = false;
    if (!illegalBuildSelectedId) {
      hint.textContent = t('lasidao.illegalBuildPickHint');
      return;
    }
    const b = (builtList || []).find((x) => x.id === illegalBuildSelectedId);
    hint.textContent = t('lasidao.illegalBuildConfirmHint', {
      name: (b && b.label) || '?',
    });
  }
  ...
  function confirmIllegalBuildPick() {
    if (!netRef || !illegalBuildSelectedId) return;
    netRef.sendAction('illegalBuildPick', {
      buildingId: illegalBuildSelectedId,
    });
    selectedBuildingId = null;
    setIllegalBuildModalOpen(false);
  }
  ...
  function renderIllegalBuildModal(game, card, mode) {
    ...
    if (mode === 'building') {
      ...
      if (
        illegalBuildSelectedId &&
        !built.some((b) => b.id === illegalBuildSelectedId)
      ) {
        illegalBuildSelectedId = null;
      }
      syncIllegalBuildFoot('building', pending);
      updateIllegalBuildHint(built);
      if (!built.length) {
      ...
      const grid = document.createElement('div');
      grid.className = 'las-illegal-build-cards';
      for (const b of built) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className =
          'las-pboard-card build is-built' +
          (illegalBuildSelectedId === b.id ? ' is-selected' : '');
        ...
        btn.onclick = () => {
          illegalBuildSelectedId =
            illegalBuildSelectedId === b.id ? null : b.id;
          renderIllegalBuildModal(game, null, 'building');
        };
        grid.appendChild(btn);
      }
    ...
    syncIllegalBuildFoot('target', null);
  ...
  function renderRedrawModal(gameParam) {
    ...
    const foot = confirmBtn && confirmBtn.closest('.las-redraw-foot');
    ...
    if (pickMode) {
      ...
      for (const c of cards) {
        ...
        if (Assets && typeof Assets.cardBackImageUrl === 'function') {
          const url = Assets.cardBackImageUrl(
            c.kind === 'building' ? 'building' : 'function'
          );
          if (url) {
  ...
  function makeExilePlayerButton(game, pid, count, onPick, resourceTotal) {
    ...
    if (resourceTotal != null) {
      ...
      resBadge.textContent = String(resourceTotal);
      resBadge.title = t('lasidao.robberyResourceTotal', { n: resourceTotal });
      btn.appendChild(resBadge);
    }
    btn.onclick = onPick;
    return btn;
  }
  ...
  function bindButtons(net) {
    ...
    if (rollBtn) {
      rollBtn.onclick = () => {
        ...
        const awaitKey = produceAwaitRollKey(lastGame, lastMeId);
        if (awaitKey) autoProduceRollSentKey = awaitKey;
        clearAutoProduceRoll();
        try {
          if (lastGame && isMercenaryRollMode(lastGame, lastMeId)) {
            const mercKey = `merc:${lastGame.round || 0}:${lastMeId}:${lastGame.currentPlayerId || ''}:${lastGame.lastPlacerId || ''}`;
    ...
    if (wishReset) wishReset.onclick = null;
    const wishConfirm = $('btn-las-wishwell-modal-confirm');
    if (wishConfirm) wishConfirm.onclick = null;
    ...
    const ibConfirm = $('btn-las-illegal-build-confirm');
    if (ibConfirm) {
      ibConfirm.onclick = () => confirmIllegalBuildPick();
    }
    ...
    // Redraw modal：仅服务端 pending 选牌时打开，不可取消中途关闭
    const redrawCancel = $('btn-las-redraw-cancel');
    if (redrawCancel) {
      redrawCancel.onclick = null;
    }
    const redrawConfirm = $('btn-las-redraw-confirm');
    if (redrawConfirm) {
      redrawConfirm.onclick = null;
    }
    ...
    if (harvestConfirm) {
      harvestConfirm.onclick = () => {
        ...
        if (harvestAction === 'wishWell') {
          if (!netRef) return;
          netRef.sendAction('allocateWishWell', {
            alloc: {
              wood: harvestCounts.wood || 0,
              stone: harvestCounts.stone || 0,
              food: harvestCounts.food || 0,
              iron: harvestCounts.iron || 0,
            },
          });
          setHarvestModalOpen(false);
          return;
        ...
        if (harvestCardId) {
          ...
          const choice =
            lastGame && lastGame.pendingEventChoice;
          if (choice && choice.needChoice === 'pickResource') {
            for (const res of RESOURCES) {
              if ((harvestCounts[res] || 0) > 0) {
