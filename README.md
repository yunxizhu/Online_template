...
- `1` 全部 · `2` Windows主机 · `3` Mac主机 · `4` 安卓APK · `5` Win纯客户端 · `6` Mac纯客户端  
  - 纯客户端（5/6）只打 `www` + 启动脚本，**不捆绑 Node**，双击用系统浏览器打开大厅（约不到 1MB）
...
- 公共地图分资源区与功能/建筑合区（各 1–6 数字格）；每轮按 `6+n` / `2+n` 摆牌（`n = 轮次 - 1`），合区由功能/建筑合堆发牌；2/4/6 号格暗置；合堆顶公开下一张类型
- 回合流程：生产阶段 → 循环掷骰派出工人 → 生产阶段结束 → 生产结算 → 建造阶段 → 使用建筑卡或功能卡 → 建造阶段结束 → 弃牌阶段 → 下一轮
- 个人版面：手牌资源默认上限 10;建筑位上限默认 3;功能手牌上限默认 3，均可扩建
